 ...
 * 
 */
public class VerticalNewsPage extends BasePage implements LoaderListener {
	...
	private VerticalSpecialItemAdapter dataAdapter;
...
//	private static VerticalNewsPage instance = null;
...
//	public static VerticalNewsPage getInstance() {
//		return instance;
//	}
	...
	public void requestDate() {
		...
		rd.getNewsSpecialList(id, new Listener4JSONObject() {
			...
			public void onError(String errorMessage) {
				...
				Lg.e(TAG, "getVerticalNewsSpecialList : " + errorMessage);
				NetWorkUtils.handlerError(errorMessage, VerticalNewsPage.this);
			}
		});
	...
	private void addGrid() {
		...
		mGrid = new Grid(VerticalNewsPage.this);
		...
		mGrid.setRowNum(5);
		mGrid.setCull(false);
		dataAdapter = new VerticalSpecialItemAdapter(this);
			...
			public void onItemClick(Actor itemView, int position,
				...
				try {
					...
					if (actName.equals("OPEN_PROGRAM_LIST")) {
						...
						options.putString("parentCatgId", parentCatgId);
						// 专题模板
					}else if(actName.equalsIgnoreCase("OPEN_NEWS_PROGRAM_LIST")){  //隐藏栏目
						actName = ACTION_NAME.OPEN_NEWS_DETAIL.name();
						options.putBoolean("is_hide", true);
						options.putString("hideCatgId", programList.get(position)
								.getFilmid());
						options.putString("parentCatgId", parentCatgId);
					}
					else {
