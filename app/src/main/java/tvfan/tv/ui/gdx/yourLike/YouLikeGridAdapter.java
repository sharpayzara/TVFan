 ...
 * 
 */
public class YouLikeGridAdapter extends GridAdapter {
	private Page page;
	List<ProgramListItem> programList = new ArrayList<ProgramListItem>();;

	public YouLikeGridAdapter(Page page) {
		this.page = page;
	}

	@Override
	public Actor getActor(int position, Actor convertActor) {
		YouLikeGridItem item = null;

		if (convertActor == null) {
			item = new YouLikeGridItem(page);
		} else {
			item = (YouLikeGridItem) convertActor;
		}
		item.setScale(1f);
	...
	class YouLikeGridItem extends Group implements IListItem, LoaderListener {
		...
		public YouLikeGridItem(Page page) {
			super(page);
			setSize(300, 400);
			setFocusAble(true);
